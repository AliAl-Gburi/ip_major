defmodule AuctionHouseWeb.Plugs.Locale do
  import Plug.Conn # Let this module know it is a plug

  @locales Gettext.known_locales(AuctionHouseWeb.Gettext) #One time retrieve the locales from the config file and store it in a @locales variable.
  ...
  def call(conn, _options) do
    case fetch_locale_from(conn) do
      ...
      locale -> # A locale is set of found in the cookies, change the locale to the prefered langage and place a new cookie
        AuctionHouseWeb.Gettext |> Gettext.put_locale(locale)
